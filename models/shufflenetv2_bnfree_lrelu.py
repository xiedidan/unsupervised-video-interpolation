    ...
    def __init__(self, inp, oup, stride):
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                ...
                # nn.BatchNorm2d(inp),
                nn.Conv2d(inp, branch_features, kernel_size=1, stride=1, padding=0, bias=True),
        ...
        self.branch2 = nn.Sequential(
            ...
            # nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=True),
            # nn.BatchNorm2d(branch_features),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
        )
...
def _shufflenetv2(arch, pretrained, progress, *args, **kwargs):
    ...
    if pretrained:
        ...
        if model_url is None:
            ...
            # remove bn parameters from state_dict
            new_state_dict = model.state_dict()
            for key in state_dict.keys():
                if ('stage' in key) and ('branch' in key):
                    arr = key.split('.')
                    serial = int(arr[3])
                    
                    if arr[2] == 'branch1':
                        if serial % 2 == 0:
                            new_state_dict['{}.{}.{}.{}.{}'.format(arr[0], arr[1], arr[2], serial//2, arr[4])] = state_dict[key]
                    elif arr[2] == 'branch2':
                        if serial == 0:
                            new_state_dict[key] = state_dict[key]
                        elif (serial > 1) and (serial % 2 == 1):
                            new_state_dict['{}.{}.{}.{}.{}'.format(arr[0], arr[1], arr[2], serial//2+1, arr[4])] = state_dict[key]
                else:
                    new_state_dict[key] = state_dict[key]
